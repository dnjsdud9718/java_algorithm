...
// 모든 노드 간의 최단 경로 -> 플로이드 와샬
// 이 문제는 모든 weight가 1이다.
public class Main {
    static int N, M;
    static int[][] src;
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int k=1; k<=N; k++){
            for(int i=1; i<=N; i++){
                for(int j=1; j<=N; j++){
                    // if(i!=j)src[i][j] = Math.min(src[i][j], src[i][k]+src[k][j]);
                    if(src[i][j] > src[i][k]+src[k][j]) src[i][j] = src[i][k]+src[k][j];
                }
            }
        }
