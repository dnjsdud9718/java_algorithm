...
class UserSolution {
    ...
    public void init(int N, int[][] mMap) {
        ...
        for (int length = 2; length <= 5; length++) {
            ...
            for (int i = 1; i + length - 1 <= n; i++) { // 세로 판단
                for (int j = 1; j <= n; j++) {
                    int hash = 0;
                    for (int k = 0; k + 1 < length; k++)
    ...
    public int unsubmergedArea(int[][] mMap, int mSeaLevel) {
        ...
        for (int i = 0; i <= n + 1; i++) { // 가능한 출발점을 모두 넣어버렸다.
            for (int j = 0; j <= n + 1; j++) {
    ...
    public int maxArea(int M, int[] mStructure, int mSeaLevel) {
        ...
        for (Candidate wall : candidate[hash]) {
            if (wall.isHorizontal) { // 가로
                int height = mStructure[0] + (wall.isReverse ? initMap[wall.r][wall.c + M - 1] : initMap[wall.r][wall.c]);// 변경되는 최종 높이
                for (int i = 0; i < M; i++)
                    modifiedMap[wall.r][wall.c + i] = height; // 배열 변경
                ret = Math.max(ret, unsubmergedArea(modifiedMap, mSeaLevel)); // 살아있는 면적 갱신
                for (int i = 0; i < M; i++)
                    modifiedMap[wall.r][wall.c + i] = initMap[wall.r][wall.c + i]; // 원상 복구
            } else { // 세로
